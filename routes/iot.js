    ...
    try {
      ...
      if (ph === undefined || ppm === undefined || temp === undefined) {
        return h
          .response({
            ok: false,
            message: "Missing required fields (ph, ppm, temp)",
          })
          .code(400);
      }
      ...
      if (!lastHumidity || now - lastHumidityFetchedAt > 3600000) {
        // 1 hour
        try {
...
const deleteAllTelemetries = {
  ...
  handler: async (request, h) => {
    try {
      ...
      console.log(
        `Successfully deleted ${result.deletedCount} telemetry records`,
      );
...
const pesticideStatus = {
  ...
  handler: async (request, h) => {
    try {
      ...
      console.error(err);
      return h
        .response({ ok: false, message: "Error fetching control" })
        .code(500);
    }
  },
};
...
module.exports = {
  ...
  register: async (server) => {
    server.route([
      ...
      getTelemetries,
      getTelemetryLatest,
      deleteAllTelemetries,
      controlPesticide,
      pesticideStatus,
      controlNutritionPump,
      nutritionStatus,
      setTimerPump,
      downloadTelemetries,
      wifiUpdate,
